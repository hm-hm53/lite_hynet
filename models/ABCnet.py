...
def swsl_resnet18(pretrained=False, **kwargs):
    ...
    state_dict = torch.load('/root/autodl-tmp/pytorch_model.bin')
...
if __name__ == "__main__":
    ...
    #
    #
    # input = torch.randn((1, 3, 1024, 1024), device=next(net.parameters()).device)
    #
    # params = parameter_count(net)[""]
    # Gflops, unsupported = flop_count(model=net, inputs=(input,))
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            start = time.time()
            raw_predictions = net(input['img'].cuda())
            # torch.cuda.synchronize()
            end = time.time()
            time_list.append(end - start)

    print(1 / (np.mean(time_list)))
